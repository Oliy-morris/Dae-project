...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add'])) {
    ...
    $uploadPath = UPLOAD_DIR . basename($imgName);
...
<?php while ($row = mysqli_fetch_assoc($paintings)): ?>
    <div class="painting">
        <img src="<?= UPLOAD_DIR . htmlspecialchars($row['image']) ?>" width="100">
        <p><strong><?= htmlspecialchars($row['title']) ?></strong></p>
        <p><?= htmlspecialchars($row['description']) ?></p>
        <p>Price: $<?= $row['price'] ?> | Discount: <?= $row['discount'] ?>%</p>
        <p><strong>Final Price: $<?= calculateFinalPrice($row['price'], $row['discount']) ?></strong></p>
        <p>Likes: <?= $row['likes'] ?></p>
